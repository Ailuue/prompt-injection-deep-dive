...
--------
  ...
  # See the difference: run the same thing with defenses OFF
...
def hardened() -> g.SupportBot:
    return g.SupportBot(input_guard=True, output_guard=True, data_defense=True)


def run_redteam(console: Console) -> int:
    bots = {"naive": g.SupportBot(), "hardened": hardened()}
    reports = {label: g.run_redteam(bot) for label, bot in bots.items()}
    ...
    console.print(
        "[dim]A low rate here means 'beat these 6 known attacks', not 'secure'. "
        "Keep adding attacks and re-running.[/dim]"
    )
    return 0
...
def parse_args(argv):
    ...
    p.add_argument("question", nargs="?", help="A question to ask. Omit for interactive chat.")
    p.add_argument("--no-defenses", action="store_true", help="Use the naive bot (show the vulnerability).")
    p.add_argument("--redteam", action="store_true", help="Fire the attack catalog at naive vs hardened and compare.")
...
def main(argv) -> int:
    ...
    def show(result: g.BotResult) -> None:
        if result.blocked:
            console.print(f"[yellow]{result.answer}[/yellow]  [dim]({result.reason})[/dim]")
        else:
            console.print(result.answer)

    if args.question:
        show(bot.ask(args.question))
        return 0
